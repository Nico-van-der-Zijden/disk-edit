...
document.getElementById('opt-changelog').addEventListener('click', function(e) {
  ...
  var changes = [
    { ver: '1.3.126', title: 'IDE64 .hdd — file data is byte-sliced (stride 4), not sequential', items: [
      ...
      'New empty .hdd images we create now match cfsfdisk byte-for-byte in every load-relevant sector (boot, partition table, bitmap, deldir, root, backup partition dir). Earlier outputs had subtle padding and self-ref attribute differences that didn\'t prevent files from loading on their own, but were worth aligning with the canonical layout',
      'View As → Hex / Disassembly / PETSCII reappear in the File menu after you leave a CFS partition. Previously the CFS branch force-disabled them without a path to re-enable, so they stayed greyed out until refresh',
    ]},
    ...
    { ver: '1.3.122', title: 'IDE64 .hdd — file-menu parity with D64, plus Insert / Remove Entry / Align / Name Case for CFS', items: [
      ...
      'Block-count edits cap at 65024 across all formats — VICE\'s directory listing truncates above that even though the 2-byte field can store 65535. Inline blocks input on the .dir-blocks column works in CFS too (double-click, type, Enter), same UX as d64',
      'Auto-select-next after Remove Entry: cursor lands on the entry that took the removed row\'s position, or the last remaining row if you removed at the end. Scroll position survives the re-render (was bouncing to top after every edit)',
      'Lots of byte-level fixes: dir-next pointer (bit-sliced across byte $14 of every slot in a directory sector) now stays intact when slots are zeroed, swapped, or overwritten — previously a Remove Entry on certain slots gave "garbage in the listing" on the next-sector contents. Short filenames now $A0-pad instead of $20 so VICE doesn\'t show trailing spaces. <<DELETED FILES>> entry is protected from rename / scratch / case-flip. CBM-DOS-only menu items (Move Up/Down, Edit T/S, View As, Lock, Splat) are hidden in CFS view — they\'d have corrupted the boot sector',
      'New partitions are created writeable by default. Deleting a partition no longer strands its LBA range — the space becomes available for a new partition immediately, and the slot can be reused too',
    ]},
    ...
    { ver: '1.3.117', title: 'IDE64 .hdd — rename, attributes, delete, import (small files)', items: [
      ...
      'Bitmap layout verified against ide.hdd — bitmaps live at partition_start + N × 4096, MSB-first, 1 = free. Every edit pushes onto the undo stack, so a wrong rename or accidental delete rolls back cleanly',
    ]},
